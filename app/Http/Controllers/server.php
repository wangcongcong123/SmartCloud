...
class WebSocket
{
    //path of log file to restore any info of error
    const LOG_PATH = '/tmp/';
    //set the maximum number of listening socket to be 9
    ...
    private function doServer()
    {
        ...
        foreach ($sockets as $socket) {
            ...
            if ($socket == $this->master) {
                ...
                $this->broadcast($msg);
//                if ($recv_msg['type'] != 'user') {
//                    $this->broadcast($msg);
//                } else {
//                    $this->sendToUser($socket,$msg);
//                }
    ...
    private function dealMsg($socket, $recv_msg)
    {
        ...
        switch ($msg_type) {
            ...
            case 'user':
                ...
                $touname = $recv_msg['touname'];
                $toid = $recv_msg['toid'];
                $fromid = $recv_msg['fromid'];
                $fromava = $recv_msg['fromava'];

                $this->sockets[(int)$socket]['touname']=$touname;
                $uname = $this->sockets[(int)$socket]['uname'];

                $response['type'] = 'user';
                $response['from'] = $uname;
                $response['touname'] = $touname;
                $response['toid'] = $toid;
                $response['fromid'] = $fromid;
                $response['content'] = $msg_content;
                $response['fromava'] = $fromava;
    ...
    private function sendToUser($mysocket, $data)
    {
        ...
        foreach ($this->sockets as $socket) {
            ...
            if ($socket['resource']['uname'] == $this->sockets[(int)$mysocket]['touname']) {
...
//        if ($this->touname!=""){
//            foreach ($this->sockets as $socket) {
//                if (  $socket['resource']['uname']==$this->touname){
//                    socket_write($socket['resource'], $data, strlen($data));
//                    break;
//                }
//            }
...
$ws = new WebSocket("0.0.0.0", "62222");